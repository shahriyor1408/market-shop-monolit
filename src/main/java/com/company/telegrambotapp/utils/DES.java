package com.company.telegrambotapp.utils;
...
class DES {
    private static class Des {
        ...
        String[] getKeys(String key) {
            ...
            System.out.println("PC-1 jadvalga qo'yildi => kalit = " + hextoBin(key) + "\n");
            System.out.println("C0" + " = " + hextoBin(key.substring(0, 7)));
            System.out.println("D0" + " = " + hextoBin(key.substring(7, 14)));

            for (int i = 0; i < 16; i++) {
                String C0 = leftCircularShift(key.substring(0, 7), shiftBits[i]);
                System.out.println("C" + (i + 1) + " = " + hextoBin(C0));
                String D0 = leftCircularShift(key.substring(7, 14), shiftBits[i]);
                System.out.println("D" + (i + 1) + " = " + hextoBin(D0));
                key = C0 + D0;
                // second key permutation
                keys[i] = permutation(PC2, key);
            }

            System.out.println("PC2 jadvaliga qoyildi!\n");
            int count = 0;
            for (String s : keys) {
                System.out.println("K" + ++count + " - " + hextoBin(s));
            }
            return keys;
        }
        ...
        String round(String input, String key, int num) {
            ...
            System.out.println("E(R0) -> E-bit selection table: " + hextoBin(temp));
            // xor temp and round key
            temp = xor(temp, key);
            // lookup in s-box table
            System.out.println("Xor(with K1 key) -> " + hextoBin(temp));

            temp = sBox(temp);
            System.out.println("After separating S blocks -> " + hextoBin(temp));

            // Straight D-box
            temp = permutation(P, temp);
            System.out.println("Permutation with P table -> " + hextoBin(temp));

            // xor
            System.out.println("L" + (num + 1) + "->" + hextoBin(right));
            left = xor(left, temp);
            System.out.println("R" + (num + 1) + "->" + hextoBin(left));
            // swapper
            System.out.println("----------------------------------\n");
            return right + left;
        }
...
//             initial permutation
            ...
            System.out.println(
                    "After splitting: L0="
                            + hextoBin(plainText.substring(0, 8))
                            + " R0="
                            + hextoBin(plainText.substring(8, 16).toUpperCase())
                            + "\n");
...
//        String text = "1111 1111 1111 1111";
        String text = "736861687269796F";
        String key = "736F6869646A6F6E";
